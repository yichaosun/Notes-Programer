...
class Solution{
  ...
  public List<List<String>> solveQueen(int target){
    ...
    Set<Integer> diagonals1 = new HashSet<Integer>();
    Set<Integer> diagonals2 = new HashSet<Integer>();
    dsf(sequence,target,0,columns,diagonals1,diagonals2);
    return result;
  }
  
  protected void dsf(int[] sequence,int target, int row,Set<Integer> columns,Set<Integer> diagonals1,Set<Integer> diagonals2){
    if(row == target){
      result.add(generateBoard(sequence));
      return;
    }
    for(int i = 0;i<target; i++){
      ...
      sequence[row] = i;
      columns.add(i);
      diagonals1.add(diaginal1);
      diagonals2.add(diagonal2);
      dsf(sequence,target,pos+1,columns,diagonals1,diagonals2);
      sequence[row] = -1;
      columns.remove(i);
      diagonals1.remove(diagonal1);
      diagonals2.remove(diagonal2);
  ...
  protected List<String> generateBoard(int[] sequence){
		List<String> board = new ArrayList<String>();
    for(int i=0 ; i<sequence.length; i++){
      char[] row = new char[sequence.length];
      Arrays.fill(row,'.');
      row[sequence[i]] = 'Q';
      board.add(new String(row));
    }
